...
from graph import NodeType, RegularDummyNode, TaskStatus
from memory import Memory, article, caches
from pprint import pprint
from typing import Dict, List
from utils.display import display_graph, display_plan
from utils.get_index import get_report_with_ref
...
class GraphRunEngine:
    ...
    def forward_one_step_not_parallel(
        self, full_step=False, select_node_hashkey=None, log_fn=None, nodes_json_file=None, *action_args, **action_kwargs
    ):
    ...
    def forward_one_step_untill_done(
        self, full_step=False, parallel=False, save_folder=None, nl=False, nodes_json_file=None, *action_args, **action_kwargs
    ):
        self.root_node.status = TaskStatus.READY
        for step in range(10000):
            logger.info("Step {}".format(step))
            ret = self.forward_one_step_not_parallel(
                full_step=False, log_fn="logs/temp/{}".format(step), nodes_json_file=nodes_json_file, *action_args, **action_kwargs
            )
...
def story_writing(input_filename, output_filename, start, end, done_flag_file, global_use_model, nodes_json_file=None):
    ...
    root_folder = "{}/{}".format(str(pathlib.Path(output_filename).parent.parent), "records")
    ...
    for item in items:
        ...
        root_node = RegularDummyNode(
            config=config,
            nid="",
            node_graph_info={"outer_node": None, "root_node": None, "parent_nodes": [], "layer": 0},
            task_info={"goal": question, "task_type": "write", "length": "determine based on the task requirements:", "dependency": []},
...
def report_writing(
    input_filename, output_filename, start, end, done_flag_file, global_use_model, engine_backend, nodes_json_file=None, today_date=None
):
    ...
    config = {
        ...
        "RETRIEVAL": {
            "execute": {
                ...
                "searcher_type": (
                    "SearXNG" if str(engine_backend).lower() == "searxng" else "SerpApiSearch"
                ),  # see recursive.executor.actions.bing_browser
                "llm_args": {
            ...
            "atom": {
                ...
                "parse_arg_dict": {"atom_think": ["think"], "update_result": ["goal_updating"]},
                "all_atom": True,
                "only_on_depend": True,
            },
            "planning": {},
...
if __name__ == "__main__":
    ...
    if args.mode == "story":
        story_writing(
            args.filename, args.output_filename, args.start, args.end, args.done_flag_file, args.model, nodes_json_file=args.nodes_json_file
        )
    else:
        report_writing(
            args.filename,
