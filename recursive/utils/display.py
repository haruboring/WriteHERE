...
import json
from graph import NodeType, TaskStatus
from utils.file_io import enum_to_json
...
def display_graph(graph, fn=None):
    ...
    if graph.outer_node.node_graph_info["root_node"] == graph.outer_node:
        ...
        display_string.append(json.dumps(graph.outer_node.result, ensure_ascii=False, indent=4, default=enum_to_json))
    ...
    for node in nodes:
        if node.node_type == NodeType.PLAN_NODE:
            display_string.append("{} <Node {}: {}> {}".format(markers, node.nid, node.task_info["goal"], markers))
            display_string.append(json.dumps(node.result, ensure_ascii=False, indent=4))
            display_string.append("\n")
        elif node.node_type == NodeType.EXECUTE_NODE:
            display_string.append(
                "{} <Level = {} Node {}: {}> {}".format(markers, node.node_graph_info["layer"], node.nid, node.task_info["goal"], markers)
            )
            display_string.append(json.dumps(node.result, ensure_ascii=False, indent=4))
            display_string.append("\n")
        else:
