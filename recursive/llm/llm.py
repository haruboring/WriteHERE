...
class OpenAIApiProxy:
    ...
    def call_embedding(self, model, text):
        ...
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                response = self.session.post(url, headers=current_headers, json=params_gpt, timeout=300, proxies=None)
    ...
    def call(
        self, model, messages, no_cache=False, overwrite_cache=False, tools=None, temperature=None, headers={}, use_official=None, **kwargs
    ):
            ...
            try:
                ...
                if system_prompt:
                    gemini_model = genai.GenerativeModel(
                        model_name=model, system_instruction=system_prompt, generation_config=generation_config
                    )
                else:
            ...
            try:
                ...
                if response.status_code not in self.RETRY_CODES:
                    ...
                    if "maximum context length is" in str(response.text) or "maximum length" in str(response.text):
                        logger.error(
                            "Error Process {} with the maximum context length exceeds. Sys messages is {}".format(model, messages[0])
                        )
                        # just return None
                        return None
