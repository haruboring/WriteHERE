...
from agent.agent_base import Agent, agent_register
from agent.prompts.base import prompt_register
from copy import deepcopy
from executor.actions import ActionExecutor
from executor.actions.bing_browser import BingBrowser
from executor.actions.register import executor_register, tool_register
from executor.agents.claude_fc_react import SearchAgent
from llm.llm import OpenAIApiProxy
from overrides import overrides
from pprint import pprint
from typing import Dict, List
from utils.file_io import make_mappings, parse_hierarchy_tags_result
...
def get_llm_output(node, agent, memory, agent_type, overwrite_cache=False, *args, **kwargs):
    ...
    system_message = prompt_register.module_dict[prompt_version]().construct_system_message(to_run_check_str=to_run_check_str)
    ...
    if task_type == "RETRIEVAL":
        ...
        if node.config["language"] == "zh":
            ...
            to_run_target_write_tasks = (
                "\n".join(
                    "Write Task{}，word count requirements：{}".format(idx, node.task_info["length"])
                    for idx, node in enumerate(depend_write_task, start=1)
                )
                if (depend_write_task is not None and len(depend_write_task) > 0)
                else "Not Provided"
    ...
    def forward(self, node, memory, *args, **kwargs) -> str:
        ...
        if inner_kwargs.get("all_atom", False):
            if not "prompt_version" in inner_kwargs:
                ...
                if (not inner_kwargs.get("only_on_depend", False)) or (len(node.node_graph_info["parent_nodes"]) > 0):
                    atom_llm_result = get_llm_output(node, self, memory, "atom", *args, **kwargs)
                    ...
                    if atom_llm_result.get("update_result", ""):
                        ...
                        logger.info("Update goal from {} to {}".format(ori_goal, node.task_info["goal"]))
            ...
            while not succ and retry_cnt < 10:
                atom_llm_result = get_llm_output(node, self, memory, "atom", retry_cnt > 0, *args, **kwargs)
            ...
            if atom_llm_result["atom_result"] == inner_kwargs["atom_result_flag"]:
                ...
                plan_result = []
                while not succ and retry_cnt < 10:
                    plan_llm_result = get_llm_output(node, self, memory, "planning", retry_cnt > 0, *args, **kwargs)
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                "Planning for {} failed, original is {}, retry {}".format(node, plan_llm_result["original"], retry_cnt)
                            )
                            retry_cnt += 1
                            continue
                    succ = True
...
class SimpleExcutor(Agent):
    ...
    def forward(self, node, memory, *args, **kwargs) -> str:
        ...
        if task_type == "RETRIEVAL" and inner_kwargs.get("react_agent", False):
            ...
            if node.config["language"] == "zh":
                ...
                to_run_outer_write_task = "COMPOSITION任务{}，字数：{}".format(outer_write_task["goal"], outer_write_task["length"])
            else:
                to_run_target_write_tasks = (
                    "\n".join(
                        "Write Task{}, word count requirements: {}".format(idx, node.task_info["length"])
            ...
            for turn_result in react_agent_result["result"]:
                ...
                execute_result.append("<web_pages_short_summary>\n{}\n</web_pages_short_summary>".format(turn_result["observation"]))
            execute_result = "\n\n".join(execute_result)

            if inner_kwargs.get("llm_merge", False):
                merge_result = self.search_merge(node, memory, execute_result, to_run_outer_write_task)
                llm_result = {
                ...
                llm_result = {"ori": react_agent_result["ori"], "result": execute_result}
        else:
            succ = False
            retry_cnt = 0
            while not succ and retry_cnt < 50:
                llm_result = get_llm_output(node, self, memory, "execute", retry_cnt > 0, *args, **kwargs)
                # 判定是否失败，如果result不为空则为成功
                succ = llm_result["result"].strip() != ""
                if not succ:
                    logger.error("Execute for {} is failed, Get Response: {}, retry_cnt={}".format(node, llm_result["original"], retry_cnt))
                    retry_cnt += 1
    ...
    def search_merge(self, node, memory, search_results, to_run_outer_write_task, *args, **kwargs):
        ...
        while not succ and retry_cnt < 50:
            ...
            if not succ:
                logger.error(
                    "Search Merge for {} is failed, Get Response: {}, retry_cnt={}".format(node, llm_result["original"], retry_cnt)
                )
                retry_cnt += 1
        if not succ:
    ...
    def forward(self, node, memory, *args, **kwargs) -> str:
        ...
        if task_type == "RETRIEVAL":
            ...
            for child in node.topological_task_queue:
                results.append("【{}】:\n {}".format(child.task_info["goal"], child.get_node_final_result()["result"]))
            results = "\n\n".join(results)
            ...
            if inner_kwargs.get("mode", "concat") == "concat":
                ...
                fa_llm_result = get_llm_output(node, self, memory, "final_aggregate", to_run_final_aggregate=results, *args, **kwargs)
