...
class Memory:
    ...
    def init(self):
        self.info_nodes = {
            self.root_node.hashkey: InfoNode(self.root_node.hashkey, self.root_node.nid, None, [], 0, self.root_node.task_info)
    ...
    def save(self, folder):
        ...
        with open("{}/memory.jsonl".format(folder), "w") as f:
            f.write(json.dumps({"article": self.article, "all_search_results": self.all_search_results}, ensure_ascii=False))
    ...
    def collect_infos(self, node_list):
        ...
        def inner(node):
            ...
            info_node = InfoNode(node.hashkey, node.nid, outer_info_node, parent_info_nodes, node.node_graph_info["layer"], info)
    ...
    def collect_node_run_info(self, graph_node):
        ...
        result = {"same_graph_precedents": same_graph_precedents_repre, "upper_graph_precedents": upper_graph_precedents_repre}
