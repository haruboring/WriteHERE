...
from executor.actions.register import executor_register
from executor.agents import ActionExecutor
from executor.agents.base_agent import BaseAgent
from executor.schema import ActionReturn, ActionStatusCode, AgentReturn, get_json_schema
    ...
    def chat(
        ...
        **kwargs,
        ...
        for turn in range(0, self.max_turn):
            ...
            cnt = 0
            while cnt < 100:
                response = self._llm.call(messages=prompt["message"], overwrite_cache=True if cnt > 0 else False, **kwargs)[0]["message"][
                    "content"
                ]
                try:
...
{content}
    ...
    # from executor.actions.google_scholar_search import GoogleScholar
    from executor.actions.python_interpreter import PythonInterpreter
    ...
    from agent.agent_base import DummyRandomPlanningAgent

    # from agent.prompts.search_think_write_push_r1.merge_search_result import MergeSearchResultZHDetailedWithOnlySummaryENR1V2
    from agent.prompts.report.merge_search_result import MergeSearchResultVFinal

    agent = DummyRandomPlanningAgent()

    to_run_root_question = (
        "20年前中美两国财富前三的富豪身价分别是多少，现在中美两国财富前三的富豪的身价分别是多少，在两个年份，中美两国身价的平均差多少"
    )
    to_run_target_write_tasks = (
        "20年前中美两国财富前三的富豪身价分别是多少，现在中美两国财富前三的富豪的身价分别是多少，在两个年份，中美两国身价的平均差多少"
    )
